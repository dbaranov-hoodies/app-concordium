...
void handleCommissionRates(uint8_t* cdata, uint8_t dataLength) {
    ...
    if (ctx_conf_baker->hasFinalizationRewardCommission) {
        ...
        fractionToPercentageDisplay(
            ...
            sizeof(ctx_conf_baker->commissionRates
...
void handleSignConfigureBaker(uint8_t* cdata, uint8_t p1, uint8_t dataLength,
    ...
    if (P1_INITIAL == p1 && isInitialCall) {
        ...
        if (offset > dataLength) {
            THROW(SWO_INVALID_TRANSACTION);  // Ensure safe access
        }
        cdata += offset;
        ...
        if (ctx_conf_baker->hasCapital) {
            ...
            if (capitalAmount == 0) {
                ...
                amountToGtuDisplay(
                    ...
                    sizeof(ctx_conf_baker->capitalRestakeDelegation
                               .displayCapital),
                    capitalAmount);
            }
        ...
        if (ctx_conf_baker->hasRestakeEarnings) {
            ...
            lengthCheck -= 1;
            if (restake == 0) {
                memmove(ctx_conf_baker->capitalRestakeDelegation.displayRestake,
                        "No", 3);
            } else if (restake == 1) {
                memmove(ctx_conf_baker->capitalRestakeDelegation.displayRestake,
                        "Yes", 4);
        ...
        if (ctx_conf_baker->hasOpenForDelegation) {
            ...
            if (openForDelegation == 0) {
                memmove(ctx_conf_baker->capitalRestakeDelegation
        ...
        if (ctx_conf_baker->hasKeys) {
            // We are expecting the signature and election verification keys
            // (each 32 bytes) and their proofs (each 64 bytes).
            if (lengthCheck != 192) {
                THROW(SWO_INVALID_TRANSACTION);
            }

            // We do not display the verification keys to the user, as they are
            // difficult for the user to verify. If need be, we can start
            // showing them by parsing the values into hex strings here.
            // Election verify key
        ...
        *flags |= IO_ASYNCH_REPLY;
        ...
        if (ctx_conf_baker->url.urlLength > dataLength) {
            ...
            *flags |= IO_ASYNCH_REPLY;
            ...
            memmove(ctx_conf_baker->url.urlDisplay, cdata,
                    ctx_conf_baker->url.urlLength);
            memmove(
                ctx_conf_baker->url.urlDisplay + ctx_conf_baker->url.urlLength,
                "\0", 1);
            updateHash((cx_hash_t*)&tx_state->hash, cdata,
                       ctx_conf_baker->url.urlLength);

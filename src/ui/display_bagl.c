 ...
 * and baker keys display.
 * - Ensures that the UI starts with the shared review transaction screens.
 * - Only displays the parts of the transaction that are set in the transaction,
 * and skips any optional fields that are not included.
 ...
 * delegation or any baker keys, then it ensures that the UI starts with the
 * shared review transaction screens. As the same method is used for the
 * pagination of the URL, this is only the case the first time it is called.
 * - If it is the final part of the URL display and there are no commission
 * rates as part of the transaction, then it displays the signing / decline
 * screens.
 * - If there are commission rates in the transaction, then it shows a continue
 * screen.
 * - If it is the final part of the URL display, then the URL screen does not

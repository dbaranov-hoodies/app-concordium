...
static cborContext_t* memo_ctx = &g_instructionContext.withDataBlob.cborContext;
...
void processNextScheduledAmount(uint8_t* buffer) {
    ...
    if (ctx->scheduledAmountsInCurrentPacket == 0) {
        // Current packet has been successfully read, but there are still more
        // data to receive. Ask the caller for more data.
        sendSuccessNoIdle();
    } else {
        // The current packet still has additional timestamp/amount pairs to be
        // added to the hash and displayed for the user.
        ...
        // If the year is too far into the future, then just fail. This is
        // needed so that we know how much space to reserve to display the date
        // time.
        if (ctx->time.tm_year > 9999) {
            THROW(SWO_INVALID_PARAM);
        }
        timeToDisplayText(ctx->time, ctx->displayTimestamp,
                          sizeof(ctx->displayTimestamp));
        if (ctx->pos + 8 > sizeof(ctx->buffer)) {
            THROW(SWO_BUFFER_OVERFLOW);
        }
        ...
        updateHash((cx_hash_t*)&tx_state->hash, buffer + ctx->pos, 8);
        ctx->pos += 8;
        amountToGtuDisplay(ctx->displayAmount, sizeof(ctx->displayAmount),
                           amount);

        // We read one more scheduled amount, so count down to keep track of
        // remaining to process.
        ctx->scheduledAmountsInCurrentPacket -= 1;

        // If it is the final schedule pair, then also allow the user to sign or
...
void handleTransferPairs(uint8_t* cdata, uint8_t dataLength,
                         volatile unsigned int* flags) {
    // Load the scheduled transfer information.
    // First 8 bytes is the timestamp, the following 8 bytes is the amount.
    // We have room for 255 bytes, so 240 = 15 * 16, i.e. 15 pairs in each
    // packet. Determine how many pairs are in the current packet.
    if (ctx->remainingNumberOfScheduledAmounts <= 15) {
        ctx->scheduledAmountsInCurrentPacket =
            ctx->remainingNumberOfScheduledAmounts;
    ...
    // Reset pointer keeping track of where we are in the current packet being
    // processed.
    ctx->pos = 0;
...
void finishMemoScheduled(volatile unsigned int* flags) {
    updateHash((cx_hash_t*)&tx_state->hash,
               &ctx->remainingNumberOfScheduledAmounts, 1);
    ctx->state = TX_TRANSFER_WITH_SCHEDULE_TRANSFER_PAIRS;
    startInitialScheduledTransferDisplay(true);
    *flags |= IO_ASYNCH_REPLY;
}

void handleSignTransferWithScheduleAndMemo(uint8_t* cdata, uint8_t p1,
                                           uint8_t dataLength,
                                           volatile unsigned int* flags,
                                           bool isInitialCall) {
    if (isInitialCall) {
    ...
    if (p1 == P1_INITIAL_WITH_MEMO &&
        ctx->state == TX_TRANSFER_WITH_SCHEDULE_INITIAL) {
        cdata += handleHeaderAndToAddress(
            cdata, dataLength, TRANSFER_WITH_SCHEDULE_WITH_MEMO,
            ctx->displayStr, sizeof(ctx->displayStr));
        ...
        // Update the state to expect the next message to contain the first
        // bytes of the memo.
        ...
        if (memo_ctx->cborLength != 0) {
            // The memo size is <=256 bytes, so we should always have received
            // the complete memo by this point;
...
void handleSignTransferWithSchedule(uint8_t* cdata, uint8_t p1, uint8_t lc,
                                    volatile unsigned int* flags,
                                    bool isInitialCall) {
    if (isInitialCall) {
        ctx->state = TX_TRANSFER_WITH_SCHEDULE_INITIAL;
    ...
    if (p1 == P1_INITIAL_PACKET &&
        ctx->state == TX_TRANSFER_WITH_SCHEDULE_INITIAL) {
        cdata +=
            handleHeaderAndToAddress(cdata, lc, TRANSFER_WITH_SCHEDULE,
                                     ctx->displayStr, sizeof(ctx->displayStr));
        ...
        // Update the state to expect the next message to contain transfer
        // pairs.
        ctx->state = TX_TRANSFER_WITH_SCHEDULE_TRANSFER_PAIRS;
        // Display the transaction information to the user (recipient address
        // and amount to be sent).

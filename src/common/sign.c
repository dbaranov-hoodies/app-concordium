...
void readCborInitial(uint8_t* cdata, uint8_t dataLength) {
    ...
    // sizeLength: number of bytes (beside the header) used to indicate the
    // payload length
...
void readCborContent(uint8_t* cdata, uint8_t contentLength) {
    ...
    switch (ctx->majorType) {
        ...
        case 0:
        case 1:
            // Type 0 and 1 fails, because we don't support integers that can't
            // fit in the initial payload.
            THROW(SWO_UNSUPPORTED_CBOR);
            break;
        default:
            THROW(SWO_INVALID_STATE);

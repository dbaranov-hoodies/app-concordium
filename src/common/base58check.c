 ...
 *    invocations, and changed the algorithm to insert a space for every 10
 * characters.
 *
...
int base58check_encode(const unsigned char* in, size_t length,
    ...
    // We need room for the version byte, length of input (has to be 32, which
    // is always the case for an address) and the first 4 bytes of the
    // SHA256(SHA256(version + in)) calculation.
    uint8_t buffer[1 + ADDRESS_LENGTH + 4];

    // Concordium uses a hardcoded version value of '1', therefore this byte is
    // not received from the computer.
    buffer[0] = 1;

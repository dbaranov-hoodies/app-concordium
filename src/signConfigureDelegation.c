...
static signConfigureDelegationContext_t* ctx =
    &g_instructionContext.signConfigureDelegation;
static tx_state_t* tx_state = &g_tx_state;

void handleSignConfigureDelegation(uint8_t* cdata, uint8_t dataLength,
    ...
    int accountTransactionHeaderAndKindLength =
        hashAccountTransactionHeaderAndKind(cdata, dataLength,
    ...
    uint8_t remainingDataLength =
        dataLength - accountTransactionHeaderAndKindLength;
    ...
    // The transaction is invalid if neither of the optional fields are
    // available.
    if (!ctx->hasCapital && !ctx->hasRestakeEarnings &&
        !ctx->hasDelegationTarget) {
    ...
    if (ctx->hasCapital) {
        ...
        if (capitalAmount == 0) {
            ...
            amountToGtuDisplay(ctx->displayCapital, sizeof(ctx->displayCapital),
                               capitalAmount);
        }
    ...
    // There was a mismatch between the transaction and the reported data
    // length.

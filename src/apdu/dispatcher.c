...
int apdu_dispatcher(const command_t* cmd, volatile unsigned int* flags,
    ...
    switch (cmd->ins) {
        ...
        case INS_SIGN_TRANSFER_WITH_MEMO:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignTransferWithMemo(cmd->data, cmd->p1, cmd->lc, flags,
                                       isInitialCall);
            break;
        case INS_SIGN_TRANSFER_WITH_SCHEDULE:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignTransferWithSchedule(cmd->data, cmd->p1, cmd->lc, flags,
                                           isInitialCall);
            break;
        case INS_SIGN_TRANSFER_WITH_SCHEDULE_AND_MEMO:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignTransferWithScheduleAndMemo(cmd->data, cmd->p1, cmd->lc,
                                                  flags, isInitialCall);
            break;
        case INS_CREDENTIAL_DEPLOYMENT:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignCredentialDeployment(cmd->data, cmd->p1, cmd->p2, cmd->lc,
                                           flags, isInitialCall);
            break;
        case INS_EXPORT_PRIVATE_KEY_LEGACY:
            LEDGER_ASSERT_NULL_CDATA;
            handleExportPrivateKey(cmd->data, cmd->p1, cmd->p2, cmd->lc, true,
                                   flags);
            break;
        case INS_EXPORT_PRIVATE_KEY_NEW:
            LEDGER_ASSERT_NULL_CDATA;
            handleExportPrivateKey(cmd->data, cmd->p1, cmd->p2, cmd->lc, false,
                                   flags);
            break;
        case INS_TRANSFER_TO_PUBLIC:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignTransferToPublic(cmd->data, cmd->p1, cmd->lc, flags,
                                       isInitialCall);
            break;
        case INS_REGISTER_DATA:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignRegisterData(cmd->data, cmd->p1, cmd->lc, flags,
                                   isInitialCall);
            break;
        case INS_PUBLIC_INFO_FOR_IP:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignPublicInformationForIp(cmd->data, cmd->p1, cmd->lc, flags,
                                             isInitialCall);
            break;
        case INS_CONFIGURE_BAKER:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignConfigureBaker(cmd->data, cmd->p1, cmd->lc, flags,
                                     isInitialCall);
            break;
        ...
        case INS_SIGN_UPDATE_CREDENTIAL:
            LEDGER_ASSERT_NULL_CDATA;
            handleSignUpdateCredential(cmd->data, cmd->p1, cmd->p2, cmd->lc,
                                       flags, isInitialCall);
            break;

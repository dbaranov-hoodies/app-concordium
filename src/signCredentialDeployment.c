...
static signCredentialDeploymentContext_t* ctx =
    &g_instructionContext.signCredentialDeploymentContext;
...
void parseVerificationKey(uint8_t* buffer, uint8_t dataLength) {
    ...
    toPaginatedHex(verificationKey, sizeof(verificationKey),
                   ctx->accountVerificationKey,
                   sizeof(ctx->accountVerificationKey));
    ctx->numberOfVerificationKeys -= 1;
}

// APDU parameters specific to credential deployment transaction (multiple
// packets protocol).
#define P1_INITIAL_PACKET 0x00  // Sent for 1st packet of the transfer.
#define P1_VERIFICATION_KEY_LENGTH 0x0A
#define P1_VERIFICATION_KEY \
    0x01  // Sent for packets containing a verification key.
...
#define P1_AR_IDENTITY \
    0x03  // Sent for the packets containing a aridentity / encidcredpubshares
          // pair.
#define P1_CREDENTIAL_DATES \
    0x04  // Sent for the packet containing the credential valid to / create at
          // dates.
...
#define P1_ATTRIBUTE_VALUE \
    0x06  // Sent for the packet containing an attribute value.
#define P1_LENGTH_OF_PROOFS \
    0x07  // Sent for the packet containing the byte length of the proofs.
...
void handleSignUpdateCredential(uint8_t* dataBuffer, uint8_t p1, uint8_t p2,
                                uint8_t lc, volatile unsigned int* flags,
                                bool isInitialCall) {
    if (isInitialCall) {
    ...
    if (p2 == P2_CREDENTIAL_INITIAL &&
        ctx->updateCredentialState == TX_UPDATE_CREDENTIAL_INITIAL) {
        uint8_t offset =
            parseKeyDerivationPath(dataBuffer, remainingDataLength);
        dataBuffer += offset;
        ...
        offset = hashAccountTransactionHeaderAndKind(
            dataBuffer, remainingDataLength, UPDATE_CREDENTIALS);
        dataBuffer += offset;
        remainingDataLength -= offset;
...
void handleSignCredentialDeployment(uint8_t* dataBuffer, uint8_t p1, uint8_t p2,
                                    uint8_t lc, volatile unsigned int* flags,
                                    bool isInitialCall) {
    if (isInitialCall) {
    ...
    if (p1 == P1_INITIAL_PACKET &&
        ...
        sendSuccessNoIdle();
               ...
               ctx->state ==
                   TX_CREDENTIAL_DEPLOYMENT_VERIFICATION_KEYS_LENGTH) {
        if (lc < 1) {
            THROW(SWO_BUFFER_OVERFLOW);
        }
        ...
        if (ctx->numberOfVerificationKeys > 0) {
            ...
            // Do not display the last verification key here. This is deferred
            // to the final UI flow.
            ctx->state = TX_CREDENTIAL_DEPLOYMENT_SIGNATURE_THRESHOLD;
            sendSuccessNoIdle();
        }
               ...
               ctx->state == TX_CREDENTIAL_DEPLOYMENT_SIGNATURE_THRESHOLD) {
        if (ctx->numberOfVerificationKeys != 0) {
            THROW(SWO_INVALID_STATE);  // Invalid state, the sender has not
                                       // sent all verification keys before
                                       // moving on.
        }
        ...
        bin2dec(ctx->signatureThreshold, sizeof(ctx->signatureThreshold),
                dataBuffer[0]);
        updateHash((cx_hash_t*)&tx_state->hash, dataBuffer, 1);
        dataBuffer += 1;
        remainingDataLength -= 1;
        // Parse the RegIdCred, but do not display it, as the user cannot
        // feasibly verify it.
        if (remainingDataLength < 48) {
            THROW(SWO_BUFFER_OVERFLOW);
        }
        if (format_hex(dataBuffer, 48, ctx->regIdCred,
                       sizeof(ctx->regIdCred)) == -1) {
            THROW(SWO_BUFFER_OVERFLOW);
        }
        ...
        numberToText((uint8_t*)ctx->identityProviderIndex,
                     sizeof(ctx->identityProviderIndex), identityProviderIndex);
        ...
        // Add the total amount of revokers to the display of threshold to get
        // "x out of y"
        ...
        if (ctx->anonymityRevocationListLength == 0) {
            // Invalid state, sender says ar identity pair is incoming, but we
        ...
        if (format_hex(dataBuffer, 4, ctx->arIdentity,
                       sizeof(ctx->arIdentity)) == -1) {
            THROW(SWO_BUFFER_OVERFLOW);
        }
        ctx->arIdentity[8] = '\0';
        updateHash((cx_hash_t*)&tx_state->hash, dataBuffer, 4);
        ...
        if (format_hex(encIdCredPubShare, 96, ctx->encIdCredPubShare,
        ...
        // We don't show these values, because only the dates on the identity
        // object can be accepted by the chain.
        ...
        // Parse attribute tag, and map it the attribute name (the display
        // text).
        ...
        updateHash((cx_hash_t*)&tx_state->hash, dataBuffer,
                   ctx->attributeValueLength);
        ...
        if (ctx->proofLength > MAX_CDATA_LENGTH) {
            ...
            // If an update credential transaction, then update state to next
            // step.
            if (p2 == P2_CREDENTIAL_CREDENTIAL &&
                ctx->updateCredentialState == TX_UPDATE_CREDENTIAL_CREDENTIAL &&
                ctx->credentialDeploymentCount > 0) {
        ...
        if (newOrExisting == 0) {
            ...
            if (base58check_encode(accountAddress, sizeof(accountAddress),
                                   ctx->accountAddress, &outputSize) == -1) {

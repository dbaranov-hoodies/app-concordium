...
#include "types.h"
#include "util.h"
static updateContract_t* ctx_update_contract =
    &g_instructionContext.updateContract;
...
void handleUpdateContract(uint8_t* cdata, uint8_t p1, uint8_t lc) {
    ...
    if (p1 == P1_INITIAL) {
        ...
        numberToText((uint8_t*)ctx_update_contract->indexDisplay,
                     sizeof(ctx_update_contract->indexDisplay), index);
        cdata += 8;
        remainingDataLength -= 8;
        ...
        numberToText((uint8_t*)ctx_update_contract->subIndexDisplay,
                     sizeof(ctx_update_contract->subIndexDisplay), subIndex);
        ...
        if (ctx_update_contract->state == UPDATE_CONTRACT_NAME_FIRST) {
            ...
            ctx_update_contract->remainingNameLength =
                ctx_update_contract->nameLength + lengthSize;

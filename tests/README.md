...
Tests use [Ragger](https://github.com/LedgerHQ/ragger) with Speculos. Shared setup is in [`conftest.py`](conftest.py), which extends `ragger.conftest.base_conftest`.
...
```shell
pytest tests/test_verify_address.py --tb=short -v --device nanosp
```
...
```shell
pytest tests/test_verify_address.py --tb=short -v --device nanosp --golden_run -s
```
